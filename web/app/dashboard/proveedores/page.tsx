...
export default function ProveedoresPage() {
  ...
  const loadProveedores = useCallback(async () => {
    ...
    try {
      ...
      setProveedores(j.data.items ?? j.data ?? []);
      setError(null);
    } catch (e) {
  ...
  const filtered = useMemo(() => {
    return proveedores.filter((p) =>
      p.nombreFiscal.toLowerCase().includes(search.toLowerCase()) ||
      p.nifCif.includes(search)
    );
  }, [proveedores, search]);
    ...
    <div>
      <header className="sticky top-0 z-40 h-16 bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 md:px-8 h-full flex items-center justify-between">
          <h1 className="font-semibold text-slate-900">Proveedores</h1>
          <button
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 py-8 flex flex-col gap-6">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            ...
            filtered.map((proveedor) => (
              <div
                key={proveedor.id}
                className="rounded-xl bg-white border border-slate-200 p-5 hover:border-accent-300 transition-colors"
              >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {proveedor.telefono && (
                    <p className="text-slate-600">📞 {proveedor.telefono}</p>
                  )}
                  {proveedor.provincia && (
                    <p className="text-slate-500">📍 {proveedor.provincia}</p>
                  )}
                </div>
              </div>
            ))
        ...
        <p className="text-sm text-slate-500 text-center">
          {loading ? 'Cargando...' : `${filtered.length} proveedor${filtered.length !== 1 ? 'es' : ''}`}
        </p>
      </main>
