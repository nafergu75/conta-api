...
export function ExpenseDetailChart({ suppliers }: ExpenseDetailChartProps) {
  const eur = (n: number) =>
    n.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' });

  // Preparar datos para el gráfico
  const chartData = suppliers.map((s, idx) => ({
    letter: s.letter,
    total: s.total,
    name: s.name,
    ...
    <div className="w-full">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-50 rounded-lg p-4 overflow-y-auto max-h-80">
          <h3 className="text-sm font-semibold text-slate-900 mb-3">Ranking de Proveedores</h3>
            ...
            {suppliers.map((supplier, idx) => (
              <div key={supplier.name} className="flex items-center gap-2 text-sm">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-slate-900 font-medium truncate">{supplier.name}</p>
                </div>
                <p className="text-slate-700 font-semibold whitespace-nowrap">{eur(supplier.total)}</p>
              </div>
            ))}
          </div>
        </div>
